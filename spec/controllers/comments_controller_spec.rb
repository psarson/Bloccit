...
RSpec.describe CommentsController, type: :controller do
  ...
  context "guest" do
   ...
   context "member user doing CRUD on a comment they don't own" do
     ...
     context "member user doing CRUD on a topic comment they don't own" do
         before do
           create_session(other_user)
         end
    ...
    context "member user doing CRUD on a comment they own" do
     ...
     context "admin user doing CRUD on a topic comment they don't own" do
       before do
         other_user.admin!
         create_session(other_user)
       end
       ...
       describe "DELETE destroy" do
         it "deletes the topic comment" do
           delete :destroy, topic_id: my_topic.id, id: my_comment.id
           count = Comment.where({id: my_comment.id}).count
           expect(count).to eq 0
         end
